...
* **Component Reuse:** Reuse existing UI primitives from `src/components/` and the shared classes (`src/index.css` tokens/primitives; `src/components/common/AppShell.css` for page spine + masthead; `src/components/medical/plate.css`, `rail.css`, `essay.css` partials: `.plate`, `.eyebrow`, `.sec-rule`, `.controls-rail` + `.rail-group/.rail-btn`, `.btn-primary/.btn-ghost` (transparent outline pills with the **magnetic-liquid** interaction from `src/utils/liquidButtons.ts`: cursor magnetic attraction with inner content parallax, elastic spring snap-back, and signature coral liquid pour from entry coordinates; never regress to flat hover fills), `.pill`, `.dot`, `.part-tip`). Controls live on the paper rail beside the plate — never overlay the 3D model. Do not create one-off ad-hoc styled variants when a canonical component already exists.
* **Co-located Stylesheets (Monolith Ban):** Every component owns one stylesheet next to its `.tsx` (`OrganDashboard.css`, `LandingPage.css`, `EvidencePage.css`, `EssayHead.css`, `OrganSelector.css`, `HotspotCallout.css`, …); classes shared across components live in `src/index.css` (primitives) or the `plate.css` / `rail.css` / `essay.css` partials in `src/components/medical/`, imported by each consuming component. Never merge styles back into one global file, and never add a new global stylesheet.
...
* **Anonymous Product:** Never place any personal name (author, student, university initials) in product copy, metadata, or footers — user directive, applies to every page and future feature.
